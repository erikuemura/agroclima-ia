  ...
  return (
    <form onSubmit={submit} className="flex flex-col sm:flex-row gap-2 max-w-lg mx-auto">
      ...
      <input
        ...
        className="sm:w-40 border border-stone-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
      />
      <button
        type="submit"
      ...
      {error && <p className="text-xs text-red-500 sm:absolute sm:mt-14">{error}</p>}
    </form>
  )
}
